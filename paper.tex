...
\begin{remark}
\cref{ass:technology} does not require differentiability. The Cobb-Douglas family
\[
F_i = A_i(S_i) L_i^{\alpha_i} \prod_{j\in S_i} X_{ij}^{\beta_{ij}}, \quad \alpha_i + \sum_{j \in S_i} \beta_{ij} = 1,
\]
satisfies \cref{ass:technology}. A CES aggregator also satisfies the assumptions provided $\sigma \leq 1$ (complements or Cobb-Douglas limit); when $\sigma > 1$ (gross substitutes), labor is no longer essential since intermediates alone can produce positive output.
\end{remark}
...
\paragraph{Interim payoff.}
Given a network profile $S$, realized state $\mu$, and equilibrium prices $P^*(S,\mu)$, firm $i$'s \textbf{payoff} from supplier set $S_i$ is:
\begin{equation}\label{eq:payoff}
\Pi_i(S_i, S_{-i}; \mu) = P_i^*(S,\mu) \cdot Y_i^* - \left( L_i^* + \sum_{j \in S_i} P_j^* X_{ij}^* \right) - \gamma |S_i|,
\end{equation}
where $(L_i^*, X_i^*)$ are cost-minimizing inputs and $Y_i^* = \theta_i(\mu) F_i(S_i, A_i, L_i^*, X_i^*)$. Under contestability (\cref{def:equilibrium}), $P_i = (1+\tau_i)\theta_i(\mu)^{-1} K_i$, so operating profits are a fixed fraction $\tau_i/(1+\tau_i)$ of revenue. The adoption cost $\gamma|S_i|$ captures the sunk cost of link formation.

\begin{remark}[Game is over supplier sets]
The extensive margin decision $S_i$ is the strategic variable. The intensive margin $(L_i, X_i)$ is induced by cost minimization once $S_i$ and prices are determined. The game is therefore over the lattice $2^{\I \setminus \{i\}}$ of supplier sets.
\end{remark}
