...
% ===== Document Metadata =====
\title{\textbf{Endogenous Production Networks under Uncertainty}}
\author{
...
\paragraph{Production Function.} Technology is given by a CES aggregator with state-dependent productivity:
\begin{equation}
Y_i = \theta_i(\mu) F_i(S_i, L_i, X_i)
\end{equation}
where $\theta_i(\mu) = e^{\varphi \mu + \eta_i}$ is a productivity shifter increasing in the state $\mu$. The core production function follows \citet{acemoglu2020endogenous}:
...
\begin{assumption}[Technological Complementarity]\label{ass:sigma}
The elasticity of substitution satisfies $\sigma < 1$.
\end{assumption}
...
Under Assumption~\ref{ass:sigma} ($\sigma < 1$), the production function $F_i$ is supermodular in $(S_i, X_i, L_i)$.
\end{lemma}

\begin{proof}[Proof Sketch]
Supermodularity requires positive cross-partials between continuous inputs and discrete increasing differences. For CES with $\sigma < 1$, the cross-partial $\partial^2 F / \partial X_j \partial X_k$ is proportional to $(1-\sigma)/\sigma > 0$. Adding a supplier raises the marginal product of existing inputs. See Appendix~\ref{app:lemma1}.
\end{proof}
