...
The extensive margin decision is made \emph{ex ante}. Firm $i$ observes $s_i$ and chooses $S_i$ to minimize expected total costs, which include a fixed organizational cost $\Gamma_i(S_i)$. The strategy of firm $i$ is a mapping $\sigma_i: \Sset_i \to 2^{\I \setminus \{i\}}$. Let $\sigma_{-i}$ denote the strategies of opponents.

Firm $i$'s objective is to minimize the expected cost function:
\begin{equation}\label{eq:objective}
    \mathcal{C}_i(S_i, s_i; \sigma_{-i}) = \E \left[ K_i(S_i, \mu, P^*(\mu, S_i, \sigma_{-i}(\s_{-i}))) + \Gamma_i(S_i) \mid s_i \right].
\end{equation}
