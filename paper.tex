...
\usepackage{mathpazo}           % Palatino: professional font for economic theory
\usepackage{microtype}          % Micro-typography for professional appearance
\usepackage{amsmath,amssymb,amsthm,mathtools}
\usepackage{bbm}
\usepackage{enumitem}
\usepackage[margin=1in]{geometry} % Tight Econometrica margins
\usepackage{setspace}
\usepackage{graphicx}
\usepackage{booktabs}
...
% TITLE PAGE
%==============================================================================
\title{\textbf{Sentiment and Supply Chains:\\ How Beliefs Shape Production Networks}\thanks{We are grateful to Daron Acemoglu, Pablo Azar, and seminar participants for detailed comments. This paper generalizes the framework of \citet{acemoglu2020endogenous} to environments with dispersed information.}}
\author{
...
\end{equation}
where $\gamma \ge 0$ is a per-link adoption cost.\footnote{Under CRS, firms make zero profits in equilibrium and $\gamma$ represents the amortized cost per unit of output from maintaining an additional supplier link. This follows the formulation of \citet{acemoglu2020endogenous}.} This two-stage structure---choose suppliers, then optimize inputs---means the strategic game is over supplier sets rather than continuous input choices.
...
Suppose \cref{ass:technology} holds. Fix $S$ and $\mu$. Then there exists a unique equilibrium price vector $P^*(S,\mu)$.
\end{proposition}

\begin{proof}
Rewriting \cref{eq:log_price_system} as $\Phi(p) = 0$ where $\Phi_i(p) = p_i - k_i - \log(1+\tau_i) + \varphi_i\mu + \eta_i$, we have $\nabla\Phi = I - \nabla_p k$. Labor essentiality implies row sums of $\nabla_p k$ are strictly below one. By the Gale-Nikaido univalence theorem \citep{acemoglu2020endogenous}, $\Phi$ is a global homeomorphism, so the system has a unique solution.
\end{proof}
...
In the Cobb-Douglas/Gaussian economy with interior equilibrium, the aggregate belief-Domar loading satisfies:
\begin{equation}\label{eq:decomposition}
    \Lambda(\s) = D'(\s) (I - B(\s))^{-1} \Phi \le \underbrace{\sum_{i=1}^n D_i(\s) \cdot \varphi_i}_{\text{Hulten/Domar term}} \times \underbrace{\frac{1}{1 - \theta(\s)}}_{\text{amplification bound}},
\end{equation}
...
\begin{proof}
In Cobb-Douglas, log-linearizing yields $dp = -(I-B)^{-1} \Phi \, d\mu$. Aggregate output responds as:
\[
\frac{d \log \text{GDP}}{d\mu} = D' \Phi + D' (I-B)^{-1} \Phi = D' (I-B)^{-1} \Phi,
\]
where the second equality uses $D' = D' (I-B)^{-1}(I-B)$. The Leontief inverse satisfies $\|(I-B)^{-1}\| \le (1 - \rho(B))^{-1}$ when $\rho(B) < 1$. Defining $\theta(\s) = \rho(B(\s))$ yields the bound.
\end{proof}
...
This appendix collects the standard results used in the main text. Proofs can be found in the cited references.
...
\begin{definition}
A function $f:L\to\R$ is \textbf{supermodular} if $f(x\vee y)+f(x\wedge y)\ge f(x)+f(y)$. A function $f:L \times T \to \R$ has \textbf{increasing differences} in $(x,t)$ if $f(x',t') - f(x,t') \ge f(x',t) - f(x,t)$ for $x' \succeq x$ and $t' \succeq t$.
\end{definition}

\begin{theorem}[Tarski's Fixed Point Theorem]\label{thm:tarski}
Let $L$ be a complete lattice and $f:L\to L$ be isotone. Then $\mathrm{Fix}(f)$ is a nonempty complete lattice.
\end{theorem}
...
\begin{definition}
Random variables $Z$ with density $f$ are \textbf{affiliated} if $f(z \vee z') f(z \wedge z') \ge f(z) f(z')$ for all $z, z'$.
\end{definition}

\begin{theorem}[Milgrom--Weber]\label{thm:milgrom_weber}
Affiliation implies MLRP; MLRP implies FOSD.
\end{theorem}

\begin{theorem}[FOSD Integration]\label{thm:fosd_int}
If $F' \ge_{\mathrm{FOSD}} F$ and $g$ is increasing, then $\E_{F'}[g] \ge \E_F[g]$.
\end{theorem}
