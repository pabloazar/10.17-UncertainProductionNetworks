...
\begin{proof}
The log-price system \cref{eq:log_price_system} defines a mapping $\Phi(p) = p - k(S, a(S), p) - b$. Labor essentiality implies that the Jacobian $I - \nabla_p k$ has row sums bounded strictly below one. By the Gale-Nikaido univalence theorem \citep{acemoglu2020endogenous}, $\Phi$ is a global homeomorphism, so the system has a unique solution.
\end{proof}
