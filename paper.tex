...
\begin{theorem}[Tarski]\label{thm:tarski}
...
\begin{theorem}[Topkis]\label{thm:topkis}
...
The Hawkins--Simon theorem gives equivalent conditions for productivity of a Leontief system. For $A\ge 0$, if $(I-A)^{-1}$ exists and has nonnegative entries, then all principal minors of $I-A$ are positive, so $I-A$ is a P-matrix. Conversely, if $I-A$ is a P-matrix, then $I-A$ is nonsingular and the inverse is nonnegative for $A\ge 0$.

\subsection{Affiliation}

Affiliation is equivalent to log-supermodularity of the joint density. Log-supermodularity is preserved under conditioning and marginalization. In particular, the conditional density $f(\mu\mid s_i)$ is log-supermodular in $(\mu,s_i)$, which yields MLRP. MLRP implies FOSD by integrating likelihood ratios. The ordering of beliefs about $s_{-i}$ follows by applying the same argument to the joint distribution of $(s_{-i},s_i)$.
