...
\begin{definition}
A matrix $B$ is a \textbf{P-matrix} if all principal minors are positive.
\end{definition}

\begin{theorem}[Hawkins--Simon]\label{thm:hawkins_simon}
For $A \ge 0$: $I-A$ is a P-matrix $\Leftrightarrow$ $(I-A)^{-1} \ge 0$ $\Leftrightarrow$ $\rho(A) < 1$.
\end{theorem}

\begin{theorem}[Gale--Nikaido]\label{thm:gale_nikaido}
If $\Phi:\R^n \to \R^n$ is $C^1$ and $D\Phi(x)$ is a P-matrix for all $x$, then $\Phi$ is a global homeomorphism.
\end{theorem}
