...
%DIF DEL /tmp/original_draft.tex   Sun Dec 14 22:13:44 2025
%DIF ADD paper.tex                 Sun Dec 14 22:06:03 2025
...
This section defines the information available to firms and records the monotone implications of affiliation that we use throughout the paper. The first is that a higher signal makes a firm more optimistic about aggregate productivity. The second is that a higher signal makes a firm more optimistic about other firms' signals, and therefore about their actions once we establish monotonicity.

Let $\mu \in \mathcal{M} \subseteq \R$ be an aggregate productivity state. Each firm $i \in \I = \{1,\ldots,n\}$ observes a private signal $s_i\in\R$. Write $s_{-i}$ for the profile of other firms' signals.
...
\begin{theorem}[Milgrom--Weber]\label{thm:mlrp}
Suppose \cref{ass:affiliated} holds. Then:
\begin{enumerate}[label=(\roman*)]
    ...
    \]
    \item MLRP implies first-order stochastic dominance: $s_i'>s_i$ implies $\pi(\cdot\mid s_i') \ge_{\mathrm{FOSD}} \pi(\cdot\mid s_i)$.
    \item Beliefs about other signals are also ordered: $s_i'>s_i$ implies
    \[
        \E[g(s_{-i})\mid s_i'] \ge \E[g(s_{-i})\mid s_i]
    \]
    for any bounded increasing function $g$.
\end{enumerate}
\end{theorem}

\begin{theorem}[FOSD integration]\label{thm:fosd_integration}
Let $F'\ge_{\mathrm{FOSD}} F$. Then $\E_{F'}[g] \ge \E_{F}[g]$ for any increasing function $g$ for which the expectations exist.
\end{theorem}

\subsection{A leading example}

\begin{example}[Gaussian common factor]\label{ex:gaussian}
Let $\mu\sim\N(\bar\mu,\sigma_\mu^2)$ and $s_i=\mu+\varepsilon_i$ with i.i.d.\ $\varepsilon_i\sim\N(0,\sigma_\varepsilon^2)$. Then $(\mu,s_1,\ldots,s_n)$ are affiliated. The posterior is
\[
\mu\mid s_i \sim \N\!\left(\frac{\sigma_\mu^2 s_i + \sigma_\varepsilon^2 \bar\mu}{\sigma_\mu^2+\sigma_\varepsilon^2},\,\frac{\sigma_\mu^2\sigma_\varepsilon^2}{\sigma_\mu^2+\sigma_\varepsilon^2}\right).
\]
In particular, the posterior mean $\E[\mu\mid s_i]$ is increasing in $s_i$.
\end{example}

%==============================================================================
\section{Production Environment}\label{sec:environment}
%==============================================================================

This section defines the production side of the economy and the unit-cost representation that summarizes firms' technology choices. The main purpose is to construct the unit cost function for each possible supplier set. This object pins down both firms' extensive margin technology choice and, through the price system, the strategic interaction across firms.

\subsection{Technology and supplier sets}

There are $n$ industries, each producing a distinct good. Industry $i$ chooses:
\begin{itemize}
    \item a supplier set $S_i \subseteq \I\setminus\{i\}$,
    \item intermediate inputs $X_i=(X_{ij})_{j\ne i}$ with $X_{ij}=0$ for $j\notin S_i$,
    \item labor $L_i\ge 0$.
\end{itemize}
...
\]
where $\theta_i(\mu)=\exp(\varphi_i\mu+\eta_i)$ with $\varphi_i\ge 0$ and $\eta_i$ is firm-specific and known to all. The term $A_i(S_i)$ captures deterministic productivity differences across supplier sets. Forming and maintaining a supplier link is costly: each adopted link contributes an adoption cost $\gamma\ge 0$.

\begin{assumption}[Production technology]\label{ass:technology}
For each $i$ and each supplier set $S_i$:
\begin{enumerate}[label=(\roman*)]
    \item $F_i(S_i,A_i,L_i,X_i)$ is continuous and strictly increasing in $(L_i,X_i)$ and strictly increasing in $A_i$.
    \item $F_i$ is homogeneous of degree one in $(L_i,X_i)$.
    \item (\emph{Labor essentiality}) $F_i(S_i,A_i,0,X_i)=0$ for all $X_i$.
\end{enumerate}
\end{assumption}

\begin{remark}
\cref{ass:technology} does not require differentiability. The CES family
\[
...
\]
satisfies \cref{ass:technology} and serves as a leading example.
\end{remark}

\subsection{Cost minimization and technology choice}

Following \citet{acemoglu2020endogenous}, we characterize firm behavior through cost minimization.

\paragraph{Intensive margin.}
Given a supplier set $S_i$, prices $P$, and productivity $A_i(S_i)$, the unit cost function is
...
\end{equation}
By homogeneity, producing $Y_i$ units costs $Y_i\cdot K_i(S_i,A_i(S_i),P)$.

\paragraph{Extensive margin.}
Given prices $P$, firm $i$ chooses a supplier set to minimize unit costs plus adoption costs:
...
\subsection{Consumers and timing}

A representative household supplies one unit of labor inelastically and consumes the $n$ goods. We assume preferences are strictly increasing, strictly concave, and homothetic. The household owns the firms and receives their profits. We keep the household side in this reduced form because our analysis focuses on unit costs, equilibrium prices, and the technology adoption problem.

Timing is as follows. Nature draws $\mu$ and signals $(s_1,\ldots,s_n)$. Each firm observes its private signal and chooses its supplier set. Given the realized network and the realized state, production and consumption take place and prices clear markets.
...
\subsection{A monotone price response}

Our strategic results use a monotonicity property of the equilibrium price mapping. Since equilibrium prices are determined by the unit cost system, it is natural to impose that when firms expand actions in the sense of adopting larger technologies, equilibrium prices weakly fall.

\begin{assumption}[Monotone price response]\label{ass:monotone_price}
Fix $\mu$. For an action profile $a=(a_1,\ldots,a_n)$, let $S(a)=(S_1,\ldots,S_n)$ denote its supplier-set component and define $P^*(a,\mu)\equiv P^*(S(a),\mu)$. If $a\succeq a'$ (in the order defined in \cref{sec:complementarities}), then $P^*(a,\mu)\le P^*(a',\mu)$ componentwise.
\end{assumption}
...
Under \cref{ass:monotone_price}, payoffs have increasing differences in $(a_i,a_{-i})$.
\end{lemma}

\begin{proof}
If $a_{-i}'\succeq a_{-i}$, then $P^*(a_i,a_{-i}',\mu)\le P^*(a_i,a_{-i},\mu)$ by \cref{ass:monotone_price}. For $a_i'\succeq a_i$, define the incremental payoff
\[
...
\textbf{Step 2:} $h$ is increasing in $s_{-i}$. Monotone $\sigma_{-i}$ implies $\sigma_{-i}(s_{-i}')\succeq\sigma_{-i}(s_{-i})$ for $s_{-i}'\ge s_{-i}$. By \cref{ass:monotone_price}, this lowers equilibrium prices. By \cref{lem:price_sc}, lower prices increase the gain from expansion, so $h$ is increasing in $s_{-i}$.
...
Under \cref{ass:affiliated,ass:technology,ass:complementarity,ass:monotone_price}, the Bayesian game admits a nonempty complete lattice of monotone Bayesian Nash equilibria. In particular, there exist greatest and least equilibria $\bar\sigma$ and $\underline\sigma$ in the lattice of monotone strategies.
\end{theorem}
...
\end{equation}
where $\theta(\s) \in (0,1)$ is an equilibrium feedback index measuring the strength of the network spillover.
\end{proposition}

\begin{proof}
(Sketch) The proof follows standard Leontief-inverse algebra. In the Cobb-Douglas case, log-linearizing around the equilibrium yields:
\[
d \log \mathrm{GDP} = \sum_i D_i \cdot d \log \theta_i + \text{(price adjustment terms)}.
\]
The price adjustment terms arise because network expansion lowers input costs, which raises output. Collecting terms, the strategic complementarity contributes a geometric series that sums to $(1 - \theta)^{-1}$, where $\theta$ depends on the spectral radius of the input-output matrix weighted by belief correlations.
\end{proof}

\paragraph{Interpretation.} In optimistic belief states, the endogenous network is denser, which pushes $\theta(\s)$ up and makes the multiplier larger. This is precisely the ``belief-adjusted Domar weight'' story: beliefs enter first order not just through the direct productivity channel but through the network channel that amplifies shocks.

When the network is exogenous (fixed $S$), $\theta = 0$ and we recover Hulten's theorem: $\Lambda = \sum_i D_i \varphi_i$. Endogenous networks under dispersed information add the amplification factor.

%==============================================================================
\section{Dynamic Extension}\label{sec:dynamic}
%==============================================================================

The static analysis describes network formation in a single period. In many applications, supplier links persist because forming or severing links is costly. This section sketches a dynamic extension that preserves the monotone structure, following \citet{van2007monotone}.
...
A firm's \textbf{state} at time $t$ is its current belief about fundamentals and peers' actions, which we summarize by the pair $(s_{it}, S_{i,t-1})$---the current signal and the inherited network. The payoff is:
\[
u_i(S_{it}, S_{-i,t}, \mu_t) - c(S_{it}, S_{i,t-1}),
\]
where $u_i$ is the period payoff from production (decreasing in unit cost) and $c(\cdot)$ is an adjustment cost:
\[
...
Under dynamic analogues of \cref{ass:affiliated,ass:technology,ass:complementarity,ass:monotone_price}, the dynamic game possesses a greatest and a least monotone Markov perfect equilibrium. In these equilibria, network expansion is monotone in the current signal: optimistic firms expand, pessimistic firms contract.
\end{theorem}

\begin{proof}
(Sketch) The proof follows \citet{van2007monotone}. The best-response operator maps monotone strategies to monotone strategies (by the single-crossing property in the static game). The space of monotone strategies is a complete lattice. By Tarski's fixed point theorem, extremal fixed points exist. Discounting ensures that the dynamic best-response is a contraction in an appropriate metric, guaranteeing uniqueness of the value function for each strategy profile.
\end{proof}

\subsection{Dynamics of beliefs and networks}

A key feature of the dynamic model is \textbf{belief updating}. As firms observe their signals and the evolution of aggregate prices, they update their beliefs about the persistent component of $\mu$. This creates a natural source of persistence: a sequence of positive signals leads to increasingly optimistic beliefs, which leads to denser networks, which lowers prices, which reinforces the expansion.

\begin{remark}[Hysteresis]
If adjustment costs are asymmetric ($\gamma^+ > \gamma^-$), the economy may exhibit hysteresis. A temporary negative shock can push the economy to the sparse equilibrium, from which it does not return even when signals recover. This is a dynamic manifestation of the equilibrium multiplicity in the static game.
\end{remark}
...
\DIFdelbegin %DIFDELCMD < \begin{theorem}[Tarski]%%%
\DIFdelend \DIFaddbegin \begin{theorem}[Tarski's Fixed Point Theorem]\DIFaddend \label{thm:tarski}
Let $L$ be a complete lattice and $f:L\to L$ be isotone \DIFaddbegin \DIFadd{(order-preserving)}\DIFaddend . Then the set of fixed points \DIFdelbegin \DIFdel{of $f$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathrm{Fix}(f) = \{x \in L : f(x) = x\}$ }\DIFaddend is a nonempty complete lattice.
\end{theorem}

\DIFaddbegin \begin{proof}
\textbf{\DIFadd{Step 1: Existence of a greatest fixed point.}}
\DIFadd{Let $A = \{x \in L : x \preceq f(x)\}$. Since $L$ is complete, $A$ has a supremum $\bar{x} = \bigvee A$. We show $\bar{x}$ is a fixed point.
}

\DIFadd{For any $x \in A$, we have $x \preceq f(x)$. Since $x \preceq \bar{x}$ and $f$ is isotone, $f(x) \preceq f(\bar{x})$. Thus $x \preceq f(\bar{x})$ for all $x \in A$, so $\bar{x} \preceq f(\bar{x})$.
}

\DIFadd{Since $\bar{x} \preceq f(\bar{x})$ and $f$ is isotone, $f(\bar{x}) \preceq f(f(\bar{x}))$. Thus $f(\bar{x}) \in A$, so $f(\bar{x}) \preceq \bar{x}$.
}

\DIFadd{Combining, $f(\bar{x}) = \bar{x}$.
}

\textbf{\DIFadd{Step 2: Existence of a least fixed point.}}
\DIFadd{Define $B = \{x \in L : f(x) \preceq x\}$ and $\underline{x} = \bigwedge B$. By a symmetric argument, $\underline{x}$ is a fixed point.
}

\textbf{\DIFadd{Step 3: The set of fixed points is a complete lattice.}}
\DIFadd{For any subset $S \subseteq \mathrm{Fix}(f)$, define $A_S = \{x \in L : x \succeq \bigvee S \text{ and } f(x) \preceq x\}$. This set is nonempty (since the greatest fixed point $\bar{x} \in A_S$) and its infimum is a fixed point that serves as the join of $S$ in $\mathrm{Fix}(f)$. Meets are constructed dually.
}\end{proof}
...
\]
\DIFaddbegin \DIFadd{A function $f:L \times T \to \R$ has }\textbf{\DIFadd{increasing differences}} \DIFadd{in $(x,t)$ if for all $x' \succeq x$ and $t' \succeq t$,
}\[
\DIFadd{f(x',t') - f(x,t') \ge f(x',t) - f(x,t).
}\]
\DIFaddend \end{definition}

\DIFdelbegin %DIFDELCMD < \begin{theorem}[Topkis]%%%
\DIFdelend \DIFaddbegin \begin{theorem}[Topkis's Monotonicity Theorem]\DIFaddend \label{thm:topkis}
\DIFaddbegin \DIFadd{Let $L$ be a lattice and $T$ a partially ordered set. }\DIFaddend If $f:L\times T\to\R$ is supermodular in $x$ and has increasing differences in $(x,t)$, then \DIFdelbegin \DIFdel{$\argmax_x f(x,t)$ }\DIFdelend \DIFaddbegin \DIFadd{$\argmax_{x \in L} f(x,t)$ }\DIFaddend is isotone in $t$ \DIFaddbegin \DIFadd{(in the strong set order)}\DIFaddend .
\end{theorem}

\DIFaddbegin \begin{proof}
\textbf{\DIFadd{Step 1: Strong set order.}}
\DIFadd{For sets $A, B \subseteq L$, we say $A \preceq_s B$ (strong set order) if for all $a \in A$ and $b \in B$, we have $a \wedge b \in A$ and $a \vee b \in B$.
}

\textbf{\DIFadd{Step 2: Key inequality.}}
\DIFadd{Let $t' \succeq t$ and suppose $x^* \in \argmax_x f(x,t)$ and $x^{**} \in \argmax_x f(x,t')$. We must show $x^* \wedge x^{**} \in \argmax_x f(x,t)$ and $x^* \vee x^{**} \in \argmax_x f(x,t')$.
}

\DIFadd{By optimality: $f(x^*,t) \ge f(x^* \wedge x^{**},t)$ and $f(x^{**},t') \ge f(x^* \vee x^{**},t')$.
}

\DIFadd{By supermodularity: $f(x^* \vee x^{**},t) + f(x^* \wedge x^{**},t) \ge f(x^*,t) + f(x^{**},t)$.
}

\DIFadd{By increasing differences: $f(x^* \vee x^{**},t') - f(x^{**},t') \ge f(x^* \vee x^{**},t) - f(x^{**},t)$.
}

\DIFadd{Combining these inequalities:
}\begin{align*}
\DIFadd{0 }&\DIFadd{\ge f(x^* \vee x^{**},t') - f(x^{**},t') }& \DIFadd{\text{(optimality of } x^{**}) }\\
&\DIFadd{\ge f(x^* \vee x^{**},t) - f(x^{**},t) }& \DIFadd{\text{(increasing differences)} }\\
&\DIFadd{\ge f(x^*,t) - f(x^* \wedge x^{**},t) }& \DIFadd{\text{(supermodularity)} }\\
&\DIFadd{\ge 0 }& \DIFadd{\text{(optimality of } x^*).
}\end{align*}
\DIFadd{All inequalities are equalities, so $x^* \wedge x^{**} \in \argmax_x f(x,t)$ and $x^* \vee x^{**} \in \argmax_x f(x,t')$.
}\end{proof}

\DIFaddend \subsection{P-matrix theory}

\DIFdelbegin \DIFdel{The Hawkins--Simon theorem gives equivalentconditions for productivity of a Leontief system.
    For $A\ge 0$, if }\DIFdelend \DIFaddbegin \begin{definition}
\DIFadd{A square matrix $B \in \R^{n \times n}$ is a }\textbf{\DIFadd{P-matrix}} \DIFadd{if all its principal minors are positive.
}\end{definition}

\begin{theorem}[Hawkins--Simon]\label{thm:hawkins_simon_proof}
\DIFadd{Let $A \ge 0$ be a nonnegative matrix. The following are equivalent:
}\begin{enumerate}[label=(\roman*)]
    \item \DIFadd{$I-A$ is a P-matrix.
    }\item \DIFaddend $(I-A)^{-1}$ exists and \DIFdelbegin \DIFdel{has nonnegativeentries, then }\DIFdelend \DIFaddbegin \DIFadd{$(I-A)^{-1} \ge 0$.
    }\item \DIFadd{The spectral radius $\rho(A) < 1$.
}\end{enumerate}
\end{theorem}

\begin{proof}
\textbf{\DIFadd{(iii) $\Rightarrow$ (ii):}}
\DIFadd{If $\rho(A) < 1$, the Neumann series $\sum_{k=0}^\infty A^k$ converges. Since $A \ge 0$, each term is nonnegative, so
}\[
\DIFadd{(I-A)^{-1} = \sum_{k=0}^\infty A^k \ge 0.
}\]

\textbf{\DIFadd{(ii) $\Rightarrow$ (i):}}
\DIFadd{For any principal submatrix $A_J$ (corresponding to index set $J$), we have $(I_J - A_J)^{-1} \ge 0$ (the Schur complement structure preserves nonnegativity). A matrix with nonnegative inverse has positive determinant, so }\DIFaddend all principal minors of $I-A$ are positive\DIFdelbegin \DIFdel{, so }\DIFdelend \DIFaddbegin \DIFadd{.
}

\textbf{\DIFadd{(i) $\Rightarrow$ (iii):}}
\DIFadd{Suppose $\rho(A) \ge 1$. By the Perron-Frobenius theorem, $A$ has a nonnegative eigenvector $v \ge 0$ with eigenvalue $\lambda = \rho(A) \ge 1$. Then $(I-A)v = (1-\lambda)v$. If $\lambda = 1$, }\DIFaddend $I-A$ \DIFdelbegin \DIFdel{is a P-matrix. Conversely, if $I-A$ }\DIFdelend is \DIFaddbegin \DIFadd{singular. If $\lambda > 1$, then $(I-A)v = (1-\lambda)v < 0$, contradicting the requirement that $(I-A)^{-1} \ge 0$ maps nonnegative vectors to nonnegative vectors.
}\end{proof}

\begin{theorem}[Gale--Nikaido Univalence]\label{thm:gale_nikaido_proof}
\DIFadd{Let $\Phi: \R^n \to \R^n$ be continuously differentiable. If the Jacobian $D\Phi(x)$ is }\DIFaddend a P-matrix \DIFdelbegin \DIFdel{, then $I-A$ is nonsingular and the inverse is nonnegative for $A\ge 0$}\DIFdelend \DIFaddbegin \DIFadd{for all $x \in \R^n$, then $\Phi$ is a global homeomorphism.
}\end{theorem}

\begin{proof}[Proof sketch]
\textbf{\DIFadd{Step 1: Local injectivity.}}
\DIFadd{A P-matrix is nonsingular (all principal minors positive implies full rank). By the inverse function theorem, $\Phi$ is a local diffeomorphism at every point}\DIFaddend .

\DIFdelbegin \subsection{\DIFdel{Affiliation}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textbf{\DIFadd{Step 2: Global injectivity.}}
\DIFadd{The key insight is that P-matrices are closed under positive diagonal scaling and satisfy a specific ``inwardness'' property. If $\Phi(x) = \Phi(y)$ for $x \ne y$, consider the path $\gamma(t) = \Phi(x + t(y-x))$. The P-matrix property forces $\gamma$ to move in all coordinates simultaneously, preventing $\gamma(0) = \gamma(1)$ unless $x = y$.
}\DIFaddend 

\DIFdelbegin \DIFdel{Affiliation is equivalent to log-supermodularity of the joint density. Log-supermodularity is preserved under conditioning and marginalization.
In particular, }\DIFdelend \DIFaddbegin \textbf{\DIFadd{Step 3: Surjectivity.}}
\DIFadd{Local injectivity plus the P-matrix property implies that $\|\Phi(x)\| \to \infty$ as $\|x\| \to \infty$ (properness). By Hadamard's theorem, a proper local homeomorphism is a global homeomorphism.
}\end{proof}

\subsection{\DIFadd{Affiliation and stochastic dominance}}

\begin{definition}
\DIFadd{Random variables $Z = (Z_1, \ldots, Z_m)$ with joint density $f$ are }\textbf{\DIFadd{affiliated}} \DIFadd{if $f$ is log-supermodular:
}\[
\DIFadd{f(z \vee z') f(z \wedge z') \ge f(z) f(z') \quad \text{for all } z, z'.
}\]
\end{definition}

\begin{theorem}[Affiliation implies MLRP]\label{thm:affiliation_mlrp}
\DIFadd{If $(Z_1, Z_2)$ are affiliated, then }\DIFaddend the conditional density \DIFdelbegin \DIFdel{$f(\mu\mid s_i)$ is log-supermodular in $(\mu,s_i)$, which yields MLRP }\DIFdelend \DIFaddbegin \DIFadd{$f(z_1 \mid z_2)$ satisfies the monotone likelihood ratio property in $z_2$}\DIFaddend .
\DIFdelbegin \DIFdel{MLRP implies FOSD by integrating likelihood ratios. The ordering of beliefs about $s_{-i}$ follows by applying the same argument to the joint distribution of $(s_{-i},s_i)$. }\DIFdelend \DIFaddbegin \end{theorem}

\begin{proof}
\DIFadd{The conditional density is $f(z_1 \mid z_2) = f(z_1, z_2) / f_{Z_2}(z_2)$. For $z_2' > z_2$ and $z_1' > z_1$:
}\begin{align*}
\DIFadd{\frac{f(z_1' \mid z_2')}{f(z_1 \mid z_2')} \cdot \frac{f(z_1 \mid z_2)}{f(z_1' \mid z_2)}
}&\DIFadd{= \frac{f(z_1', z_2') \cdot f(z_1, z_2)}{f(z_1, z_2') \cdot f(z_1', z_2)}.
}\end{align*}
\DIFadd{Define $z = (z_1, z_2)$ and $z' = (z_1', z_2')$. Then $z \vee z' = (z_1', z_2')$ and $z \wedge z' = (z_1, z_2)$. By log-supermodularity:
}\[
\DIFadd{f(z_1', z_2') \cdot f(z_1, z_2) \ge f(z_1, z_2') \cdot f(z_1', z_2).
}\]
\DIFadd{Thus the ratio is $\ge 1$, which is the MLRP condition.
}\end{proof}

\begin{theorem}[MLRP implies FOSD]\label{thm:mlrp_fosd}
\DIFadd{If $f(\cdot \mid s')$ dominates $f(\cdot \mid s)$ in the likelihood ratio order for $s' > s$, then $F(\cdot \mid s') \ge_{\mathrm{FOSD}} F(\cdot \mid s)$.
}\end{theorem}

\begin{proof}
\DIFadd{MLRP means $f(\mu \mid s') / f(\mu \mid s)$ is increasing in $\mu$. Define $\Lambda(\bar{\mu}) = \int_{-\infty}^{\bar{\mu}} f(\mu \mid s') d\mu / \int_{-\infty}^{\bar{\mu}} f(\mu \mid s) d\mu$. 
}

\DIFadd{Since the likelihood ratio is increasing, $\Lambda(\bar{\mu})$ is increasing in $\bar{\mu}$. We have $\Lambda(-\infty) = 0$ and $\Lambda(\infty) = 1$ (both CDFs integrate to 1).
}

\DIFadd{For FOSD, we need $F(\bar{\mu} \mid s') \le F(\bar{\mu} \mid s)$, i.e., $\int_{-\infty}^{\bar{\mu}} f(\mu \mid s') d\mu \le \int_{-\infty}^{\bar{\mu}} f(\mu \mid s) d\mu$.
}

\DIFadd{Equivalently: $\int_{-\infty}^{\bar{\mu}} [f(\mu \mid s') - f(\mu \mid s)] d\mu \le 0$.
}

\DIFadd{The integrands start negative (for small $\mu$, the ratio is below its average of 1) and end positive, crossing zero once. The integral is the area under the difference, which is nonpositive up to every point $\bar{\mu}$.
}\end{proof}

\begin{theorem}[FOSD Integration]\label{thm:fosd_int}
\DIFadd{If $F' \ge_{\mathrm{FOSD}} F$ and $g$ is increasing, then $\E_{F'}[g(X)] \ge \E_F[g(X)]$.
}\end{theorem}

\begin{proof}
\DIFadd{By integration by parts:
}\[
\DIFadd{\E_F[g(X)] = \int_{-\infty}^\infty g(x) dF(x) = g(\infty) - \int_{-\infty}^\infty g'(x) F(x) dx.
}\]
\DIFadd{Since $F'(x) \le F(x)$ for all $x$ (FOSD) and $g'(x) \ge 0$ (increasing):
}\[
\DIFadd{\E_{F'}[g(X)] = g(\infty) - \int_{-\infty}^\infty g'(x) F'(x) dx \ge g(\infty) - \int_{-\infty}^\infty g'(x) F(x) dx = \E_F[g(X)].
}\]
\end{proof}

\subsection{\DIFadd{Proof of price existence (Proposition~\ref{prop:price_existence})}}

\begin{proof}[Full proof]
\DIFadd{Define the mapping $\Phi: \R^n \to \R^n$ by
}\[
\DIFadd{\Phi_i(p) = p_i - k_i(S_i, a_i(S_i), p) - b_i,
}\]
\DIFadd{where $b_i = \log(1+\tau_i) - (\varphi_i \mu + \eta_i)$. Equilibrium prices solve $\Phi(p) = 0$.
}

\textbf{\DIFadd{Step 1: Jacobian structure.}}
\DIFadd{The Jacobian is $D\Phi(p) = I - J_{k,p}(p)$, where $[J_{k,p}]_{ij} = \partial k_i / \partial p_j$.
}

\DIFadd{By Shepard's lemma, in the differentiable case:
}\[
\DIFadd{\frac{\partial k_i}{\partial p_j}(p) = \frac{P_j X_{ij}^*(P)}{K_i(S_i, A_i, P)} = \text{cost share of input } j \text{ in sector } i.
}\]

\textbf{\DIFadd{Step 2: Row sum bound.}}
\DIFadd{Labor essentiality implies that labor has a positive cost share in every sector. Thus:
}\[
\DIFadd{\sum_{j=1}^n \frac{\partial k_i}{\partial p_j}(p) = \text{(total intermediate cost share)} < 1.
}\]
\DIFadd{Let $\kappa = \sup_{i,p} \sum_j \partial k_i / \partial p_j < 1$.
}

\textbf{\DIFadd{Step 3: P-matrix verification.}}
\DIFadd{Since $J_{k,p} \ge 0$ (cost shares are nonnegative) and row sums are bounded by $\kappa < 1$, we have $\rho(J_{k,p}) \le \kappa < 1$. By \mbox{%DIFAUXCMD
\cref{thm:hawkins_simon_proof}}\hspace{0pt}%DIFAUXCMD
, $I - J_{k,p}$ is a P-matrix.
}

\textbf{\DIFadd{Step 4: Global homeomorphism.}}
\DIFadd{By \mbox{%DIFAUXCMD
\cref{thm:gale_nikaido_proof}}\hspace{0pt}%DIFAUXCMD
, $\Phi$ is a global homeomorphism. Therefore $\Phi(p) = 0$ has a unique solution.
}\end{proof}
\DIFaddend 
